...
- 支持 Discord webhook 发送，也支持 bot token + channelId 做频道反控；`adminIds` 可填单个 Discord 用户 ID、逗号分隔字符串或数组。QQ 群反控分两级权限：`!help`/`!list`/`!day`/`!rules`/`!version`/`!uptime`/`!ip`/`!roll`/`!运势`/`!id`/`!绑定` 全体群友可用；`!tps`/`!backup`/`!save`/`!seed`/`!weather`/`!stop`/`!restart`/`!cmd`/`!ai`（查当前 AI 模型与可切换预设）仅群主、群管理员（QQ 群身份自动识别）或 `qq.adminIds` 白名单可用。
...
- QQ AI 支持当前消息、引用消息和合并转发中的视频及语音：原视频由 Qwen3.7 Flash 以 `fps=1.0` 覆盖完整时间轴；Qwen Audio ASR 负责视频音轨/QQ 语音文字，Qwen3.5-Omni 可选理解唱歌、音乐与环境声。引用语音发送 `!转写` 只做 ASR，发送 `!听语音` 或带语音 `!问`/`@机器人` 才并行听声音；Silk 优先由 OneBot 转码、失败时本机解 WAV，最终由 `ai.provider`（公版默认 DeepSeek）汇总。机器人不会自动识别全群语音，ASR/Omni 证据正文不进入多轮历史，但客群按群号与权限保留最近 6 轮用户问题和 AI 结论，因此纠正后的音乐名称能在同一客群下一次引用中继承；所有媒体证据均不能成为服务器操作授权。详见 [视频、语音识别与 DeepSeek 汇总](qq-video-audio-ai.md)。
- 跨群共享 AI 知识库独立于客群短期历史：管理员在主群引用机器人回答或原语音后，直接写“正确歌名是《歌名》”即可自动记住；也可用 `!知识库记住 主题 => 结论`（引用语音时自动绑定回复与音频 SHA-256 指纹）。客群 `@机器人` 会先按文本和音频指纹检索相似结论，再交给 DeepSeek 做最终判断。默认只保存短文本通用知识，不保存日志、配置、密钥、群号、QQ 号或原始媒体；普通群友只读。查询/删除命令为 `!知识库查询 关键词`、`!知识库删除 关键词`，文件默认在 `logs/ai-shared-knowledge.jsonl`。
- 定时世界备份会尽量走 RCON `save-off/save-all flush/save-on`，失败时仍会 zip 当前世界；手动 `!backup`/`!备份` 只发一条简洁完成回执，并会抑制 backupWatch 的重复通知。
- 备份调度兼容固定间隔模式，也可启用 `backupSchedule.activityAware`：有人在线按 `activeIntervalMinutes` 备份，最后一人下线后去抖补一次收尾，空服不再重复周期打包；`offlineFinalMinAgeMinutes` 可避免刚完成的归档被立即重复复制。
