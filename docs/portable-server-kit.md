...
- QQ AI 支持当前消息、引用消息和合并转发中的视频：原视频可用时由 Qwen3.7 Flash 以 `fps=1.0` 覆盖完整时间轴，可选 Qwen Audio ASR 并行读取音轨，最后由 `ai.provider`（公版默认 DeepSeek）汇总；原视频不可取才降级为 1–3 张关键帧。画面报告与转写均按不可信媒体数据处理，不能成为服务器操作授权。详见 [视频画面、音轨与 DeepSeek 汇总](qq-video-audio-ai.md)。
