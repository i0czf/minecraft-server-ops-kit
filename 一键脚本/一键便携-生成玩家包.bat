...
powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Write-Host '========================================'; Write-Host '  生成玩家包'; Write-Host '========================================'; Write-Host '生成 PCL 等启动器可用的玩家客户端包。'; Write-Host ''"
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0..\tools\build-portable-player-pack.ps1" -ConfigPath "%~dp0..\tools\portable-pack.json"
set "code=%ERRORLEVEL%"

if "%code%"=="0" (
    powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "Write-Host ''; Write-Host '[完成] 玩家包已生成。'"
) else (
